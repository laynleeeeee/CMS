...
SalesTableHandler = function(totalHandler) {
	...
	this.showItems = function(setting, $table, $txtBox) {
		var itemStockCode = $.trim($txtBox.val());
		var isSerial = $($table).parent().attr("class").indexOf("tblSerial") !== -1;
		var uri = contextPath+"/getRetailSerialItem/getRetailItems?isSerialized="+isSerial
			+"&isActive=true&stockCode="+encodeURIComponent(itemStockCode);
	...
	if ($.trim(item.stockCode) != "") {
		...
		if (typeof $warehouse != "undefined" && item.id > 0) {
			var uri = contextPath + "/getWarehouse/withES?companyId="
					+ companyId + "&itemId=" + item.id +"&divisionId="+divisionId;
			...
			success : function(item) {
				if (item != null) {
					if (setting.disableDuplicateStockCode) {
						validateStockCode($table, $txtBox, item.item, doAfterProcess);
					} else {
						processStockCode($table, $txtBox, item.item, doAfterProcess);
					}
				}
			},
...
function computeDiscount($input, itemDiscountTypeId, isOtherCharges) {
	...
	if (itemDiscountTypeId == DT_PERCENTAGE) {
		...
		discount = quantity < 0 ? (discountValue * -1) : discountValue;
...
function getGrossAmount($input, isOtherCharges) {
	...
	if (isOtherCharges) {
		...
		srp = $($input).closest("tr").find(".srp").html();
	}

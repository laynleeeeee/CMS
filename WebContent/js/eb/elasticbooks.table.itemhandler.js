...
ItemTableHandler = function (totalHandler) {
	...
	this.showItems = function (setting, $table, $txtBox) {
		...
		if (IS_EXCLUDE_FG != undefined) { // Check if we should include or exclude the finished goods.
			uri += "&isExcludeFG="+IS_EXCLUDE_FG;
		}
		$($txtBox).autocomplete({
			source: uri,
...
function getLatestUC($txtBox, itemId) {
	...
	var warehouseId = $("#warehouseId").val();
	if (supplierAccountId != "" && supplierAccountId != null && typeof supplierAccountId !== "undefined") {

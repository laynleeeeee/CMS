...
SalesTableHandler = function(totalHandler) {
	...
	this.showItems = function(setting, $table, $txtBox) {
		...
		var uri = contextPath+"/getRItems/filterItems?stockCode="+encodeURIComponent(stockCode);
...
function processStockCode($table, $txtBox, item, doAfterProcess) {
	...
	if ($.trim(item.stockCode) != "") {
		...
		if (typeof $warehouse != undefined && item.id > 0) {
			...
			var uri = contextPath+"/getWarehouse/withES?companyId="+companyId+"&itemId="+item.id+"&divisionId="+divisionId;
			if (warehouseId != undefined) {
				uri += "&warehouseId="+warehouseId;
			}
...
function populateSrp(companyId, itemId, $txtBox) {
	var uri = contextPath+"/getItemSrp/withRate?companyId="+companyId+"&itemId="+itemId;
	var divisionId = $("#divisionId").val();
	if (divisionId != undefined) {
		uri += "&divisionId="+divisionId;
	}
	...
	$.ajax({
		...
		success : function(item) {
			if (item != null) {
				var $srp = $($txtBox).closest("tr").find(".grossAmount");
				$srp.val(formatDecimalPlaces(item.sellingPrice));
			}
		},
		error : function(error) {
			console.log(error);

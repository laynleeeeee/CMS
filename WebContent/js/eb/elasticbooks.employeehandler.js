	...
	$.fn.editableItem = function (otherParams) {
				...
				if ($property != null) {
					...
					if ($property.varType == "double") {
						if($property.name != "unitCost") {
							...
							value = accounting.toFixed(value, 6);
						}
					} else if ($property.varType == "string") {
						value = value.replace(/'/g, "&#39;");
						// console.log("decoded URIComponent : " + value);
					}
		...
		function buildCell (cellData, cssClass, headerCell) {
			...
			if (editorType == "label") {
				...
				cellContent = "<select class='"+cssClass+"'>";
				cellContent += "<option value=1>SINGLE</option>";
				cellContent += "<option value=2>MARRIED</option>"
				cellContent += "</select>";

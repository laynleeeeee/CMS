-- Description: Stored proceedure that will get the income statement per account.

Delimiter //
DROP procedure if exists GET_INCOME_STATEMENT;//
CREATE procedure GET_INCOME_STATEMENT (IN IN_COMPANY_ID INT, IN IN_ACCOUNT_ID INT, 
...
UNION ALL
-- Cash Sales -- Sales Discount Config
...
UNION ALL
-- Cash Sales -- Cost of Sales Config
...
UNION ALL 
-- Cash Sales -- Other charges DEBIT
...
UNION ALL 
-- Cash Sales -- Other charges CREDIT
...
UNION ALL
-- Account Sales -- Sales Discount Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, SUM(COALESCE(DISCOUNT, 0)) AS DEBIT, 0 as CREDIT
FROM ACCOUNT_SALE_ITEM ASI
INNER JOIN AR_TRANSACTION ART ON ART.AR_TRANSACTION_ID = ASI.AR_TRANSACTION_ID
INNER JOIN ITEM I ON I.ITEM_ID = ASI.ITEM_ID
...
UNION ALL
-- Account Sales -- Cost of Sales Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME,  0 as DEBIT, SUM(QUANTITY * COALESCE(UNIT_COST, 0)) as CREDIT
FROM ACCOUNT_SALE_ITEM ASI
INNER JOIN AR_TRANSACTION ART ON ART.AR_TRANSACTION_ID = ASI.AR_TRANSACTION_ID
INNER JOIN ITEM I ON I.ITEM_ID = ASI.ITEM_ID
...
UNION ALL
-- Account Sales -- Other Charges DEBIT
...
UNION ALL
-- Account Sales -- Other Charges CREDIT
...
-- Cash Sales Return -- Sales Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, 0 AS DEBIT, SUM(QUANTITY * SRP) as CREDIT
...
UNION ALL
-- Cash Sales Return -- Sales Discount Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, SUM(COALESCE(DISCOUNT, 0)) AS DEBIT, 0 as CREDIT
FROM CASH_SALE_RETURN_ITEM CSRI
INNER JOIN CASH_SALE_RETURN CSR ON CSR.CASH_SALE_RETURN_ID = CSRI.CASH_SALE_RETURN_ID
INNER JOIN ITEM I ON I.ITEM_ID = CSRI.ITEM_ID
...
UNION ALL
-- Cash Sales Return -- Cost of Sales Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME,  0 as DEBIT, SUM(QUANTITY * COALESCE(UNIT_COST, 0)) as CREDIT
FROM CASH_SALE_RETURN_ITEM CSRI
INNER JOIN CASH_SALE_RETURN CSR ON CSR.CASH_SALE_RETURN_ID = CSRI.CASH_SALE_RETURN_ID
INNER JOIN ITEM I ON I.ITEM_ID = CSRI.ITEM_ID
...
-- Account Sales Return -- Sales Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, 0 AS DEBIT, SUM(QUANTITY * SRP) as CREDIT
FROM ACCOUNT_SALE_ITEM ASI
INNER JOIN AR_TRANSACTION ART ON ART.AR_TRANSACTION_ID = ASI.AR_TRANSACTION_ID
INNER JOIN ITEM I ON I.ITEM_ID = ASI.ITEM_ID
...
UNION ALL
-- Account Sales Return -- Sales Discount Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, SUM(COALESCE(DISCOUNT, 0)) AS DEBIT, 0 as CREDIT
FROM ACCOUNT_SALE_ITEM ASI
INNER JOIN AR_TRANSACTION ART ON ART.AR_TRANSACTION_ID = ASI.AR_TRANSACTION_ID
INNER JOIN ITEM I ON I.ITEM_ID = ASI.ITEM_ID
...
UNION ALL
-- Account Sales Return -- Cost of Sales Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, 0 as DEBIT, SUM(QUANTITY * COALESCE(UNIT_COST, 0)) as CREDIT
FROM ACCOUNT_SALE_ITEM ASI
INNER JOIN AR_TRANSACTION ART ON ART.AR_TRANSACTION_ID = ASI.AR_TRANSACTION_ID
INNER JOIN ITEM I ON I.ITEM_ID = ASI.ITEM_ID
...
UNION ALL 
-- Stock Adjustment In -- Cost of Sales Config
...
UNION ALL 
-- Stock Adjustment Out -- Cost of Sales Config
SELECT ACCT.ACCOUNT_ID, ACCT.ACCOUNT_NAME, SUM(-(QUANTITY * COALESCE(UNIT_COST, 0))) as DEBIT, 0 as CREDIT
FROM STOCK_ADJUSTMENT_ITEM SAI
INNER JOIN STOCK_ADJUSTMENT SA ON SA.STOCK_ADJUSTMENT_ID = SAI.STOCK_ADJUSTMENT_ID
INNER JOIN STOCK_ADJUSTMENT_TYPE SAT ON SAT.STOCK_ADJUSTMENT_TYPE_ID = SA.STOCK_ADJUSTMENT_TYPE_ID

...
/**
 * Allocate the unit cost of the item using weighted average method.

 */

public class WeightedAveItemAllocator<T extends InventoryItem> {
	private static final Logger logger = Logger.getLogger(WeightedAveItemAllocator.class);
	 ...
	 */
	public WeightedAveItemAllocator (ItemDao itemDao, ItemService itemService, int itemId, int warehouseId, Date date) {
		this.itemId = itemId;
		...
		int companyId = itemService.getWarehouse(warehouseId).getCompanyId();
		// Get remaining quantity with average cost
		StockcardDto stockcardDto = itemService.getStockcardBeginningBal(companyId, warehouseId, itemId).iterator().next();
		unusedStocks.add(ReceivedStock.getInstanceOf(currentDate, itemId, stockcardDto.getQuantity(), stockcardDto.getUnitCost()));

...
/**
 * Class that will handle the generation of Inventory Listing Report.

 *
 */
@Controller
@RequestMapping("/rInventoryListingPDF")
public class RInventoryListingJR {
	@Autowired
	...
	private static Logger logger = Logger.getLogger(RInventoryListingJR.class);

	@RequestMapping(value="/showParams", method = RequestMethod.GET)
	public String showPI(Model model, HttpSession session){
		logger.info("Showing the search page for Inventory Listing Report.");
		User user = CurrentSessionHandler.getLoggedInUser(session);
		model.addAttribute("categories", piService.getActiveCategories());
		...
		return "RInventoryListingReport.jsp";
	}
	...
	public String generateReport(@RequestParam(value="itemCategoryId") Integer itemCategoryId,
			...
			@RequestParam(value="isAveraging", required=true) boolean isAveraging,
		...
		logger.info("Generating the Inventory Listing Report for company: "+companyId
				+" with warehouse: "+warehouseId+" as of "+asOfDate);
		logger.info("Generating the report using Averaging Method.");
		JRDataSource dataSource = invListingService.generateInventoryListing(divisionId, itemCategoryId,
					stockOptionId, companyId, warehouseId, status, 1, orderBy, asOfDate);
		return generateReport(model, session, dataSource, formatType, companyId, divisionId,
				asOfDate, itemCategoryId, warehouseId, isFirstNameFirst);
	}

	private String generateReport(Model model, HttpSession session, JRDataSource dataSource, String formatType,
			Integer companyId,Integer divisionId, String asOfDate, Integer itemCategoryId, Integer warehouseId, Boolean isFirstNameFirst) {
		User user = CurrentSessionHandler.getLoggedInUser(session);
		...
		return "RInventoryListingReport.jasper";
	}
}